...
export const SITE_CONFIG = {
  brand: "Interactive Visualization",
...
type AnimStyle = "fade-blur" | "slide-up" | "spring-pop" | "split-reveal" | "scramble" | "stagger-wave";

function getVariants(style: AnimStyle) {
  const container = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.11, delayChildren: 0.18 } } };

  const items: Record<AnimStyle, { hidden: object; show: object }> = {
    "fade-blur": {
  ...
  return { container, item: items[style] ?? items["fade-blur"] };
}
...
function useScramble(text: string, active: boolean) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      frame.current++;
      if (frame.current < iterations) raf.current = requestAnimationFrame(animate);
...
export default function Hero() {
  ...
  // key forces remount → animation re-runs whenever variant or animation style changes
...
function HeroSplit({ animStyle }: { animStyle: AnimStyle }) {
  ...
  const headline0 = useScramble(SITE_CONFIG.headline[0], animStyle === "scramble");
  const headline1 = useScramble(SITE_CONFIG.headline[1], animStyle === "scramble");
  const headline2 = useScramble(SITE_CONFIG.headline[2], animStyle === "scramble");
      ...
      <motion.div className="relative z-10 max-w-7xl mx-auto px-6 pt-28 pb-16 w-full" style={{ y, opacity: op }}>
        <div className="grid lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_460px] gap-14 items-center">
          <motion.div variants={container} initial="hidden" animate="show"
            ...
            {/* Headline */}
            <motion.h1 variants={item} className="font-light leading-[0.88] tracking-tight mb-7"
              style={{ fontFamily: "var(--font-display)", fontSize: "clamp(3.2rem,6.5vw,6rem)" }}>
              {animStyle === "stagger-wave"
                ? <StaggerWords text={SITE_CONFIG.headline[0]} />
                : <>{headline0}</>
              }<br />
              <span className="text-gradient italic">
                {animStyle === "stagger-wave"
                  ? <StaggerWords text={SITE_CONFIG.headline[1]} />
                  : <>{headline1}</>}
              </span><br />
              {animStyle === "stagger-wave"
                ? <StaggerWords text={SITE_CONFIG.headline[2]} />
                : <>{headline2}</>
              }
            </motion.h1>

            {/* Sub */}
            <motion.p variants={item} className="text-muted-foreground leading-relaxed mb-9 max-w-[420px]"
            ...
            <motion.div variants={item} className="flex items-center gap-10 pt-8 border-t border-border/40">
              {SITE_CONFIG.stats.map(s => (
                <div key={s.label}>
                  <div className="font-light leading-none mb-1" style={{ fontFamily: "var(--font-display)", fontSize: "1.7rem" }}>{s.value}</div>
...
function HeroCentered({ animStyle }: { animStyle: AnimStyle }) {
  ...
  return (
    <motion.section key="centered"
      ...
      <HeroBg />
      <motion.div className="relative z-10 max-w-4xl mx-auto px-6 pt-28 pb-16"
        ...
        <motion.h1 variants={item} className="font-light leading-[0.9] tracking-tight mb-8"
          style={{ fontFamily: "var(--font-display)", fontSize: "clamp(3.5rem,8vw,7.5rem)" }}>
          {animStyle === "stagger-wave" ? <StaggerWords text={SITE_CONFIG.headline[0]} /> : headline0}{" "}
          <span className="text-gradient italic">
            {animStyle === "stagger-wave" ? <StaggerWords text={SITE_CONFIG.headline[1]} /> : headline1}
          </span><br />
          {animStyle === "stagger-wave" ? <StaggerWords text={SITE_CONFIG.headline[2]} /> : headline2}
        </motion.h1>
        ...
        <motion.div variants={item} className="flex items-center justify-center gap-12 mt-16 pt-8 border-t border-border/30">
          {SITE_CONFIG.stats.map(s => (
            <div key={s.label} className="text-center">
              <div className="font-light mb-0.5" style={{ fontFamily: "var(--font-display)", fontSize: "2rem" }}>{s.value}</div>
  ...
  return (
    <motion.section key="bold"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-40 w-full">
        <motion.h1 className="font-light leading-[0.82] tracking-[-0.02em] mb-10"
          style={{ fontFamily: "var(--font-display)", fontSize: "clamp(5rem,13vw,13rem)" }}
          variants={container} initial="hidden" animate="show"
          {...(animStyle === "split-reveal" ? { style: { fontFamily: "var(--font-display)", fontSize: "clamp(5rem,13vw,13rem)", perspective: 1000 } } : {})}>
...
function HeroVisual() {
  return (
    <div className="relative">
      <motion.div className="absolute -top-7 -left-8 z-20 flex items-center gap-3 px-4 py-3 rounded-2xl shadow-xl"
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-border/30 aspect-[4/3]"
        ...
        <svg className="absolute inset-0 w-full h-full opacity-50" viewBox="0 0 400 300" fill="none">
          <g stroke="hsl(var(--primary))" strokeWidth="0.6" opacity="0.5">
            ...
            {[...Array(16)].map((_,i) => <line key={i} x1="130" y1={55+i*13} x2="270" y2={55+i*13} opacity="0.18" />)}
            <line x1="200" y1="8" x2="130" y2="55" opacity="0.3" /><line x1="200" y1="8" x2="270" y2="55" opacity="0.3" />
          </g>
        ...
        <motion.div className="absolute left-0 right-0 h-px"
          ...
          animate={{ top: ["6%","94%","6%"] }} transition={{ duration: 7, repeat: Infinity, ease: "linear" }} />
      ...
      <motion.div className="absolute -bottom-5 -right-7 z-20 px-4 py-3 rounded-2xl shadow-xl"
        style={{ background: "hsl(var(--card)/0.82)", border: "1px solid hsl(var(--border)/0.5)", backdropFilter: "blur(16px)" }}
        ...
        <div className="flex items-center gap-1">
          {["A","M","K","J","R"].map((l,i) => (
            <div key={i} className="w-6 h-6 rounded-full flex items-center justify-center text-[8px] font-medium"
              style={{ background: "hsl(var(--primary)/0.15)", border: "1px solid hsl(var(--primary)/0.2)", color: "hsl(var(--primary))" }}>{l}</div>
          ))}
